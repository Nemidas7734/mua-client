      ...
      <div className="flex items-center">
        {isClient && user ? (
          role === 'artist' ? (
            ...
            <button onClick={handleLogout} className="text-white font-semibold bg-pink-500 hover:bg-pink-600 px-4 py-1 rounded-full">
              Logout
            </button>
          )
        ) : (
          <Link href="/pages/login" className="text-white font-semibold bg-pink-500 hover:bg-pink-600 px-4 py-1 rounded-full">
            Login
          </Link>
        )}
      </div>
